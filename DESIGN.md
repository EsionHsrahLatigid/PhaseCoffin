...
The editor uses `juce-ehl-design-module` as the source of truth for the strict DHN9 simple monochrome 8-bit contract: flat four-level palette only (`#050505`, `#2A2A2A`, `#8A8A86`, `#F2F2F0`), 4 px base grid / 8 px major spacing, shared product header, one shared divider, and controls starting below the 64 px header. There is no full-canvas grid, tagline, package ID, decorative motif, fake visualizer, meter, panel, outer frame, or parameter-driven atmospheric paint. The default size is 640 x 360 and the minimum is 512 x 320.
